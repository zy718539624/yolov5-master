...
def test(data,
         ...
         batch_size=16,
         imgsz=640,
         conf_thres=0.1,    # 原本是0.001
         iou_thres=0.4,  # for NMS  原本是0.6
         save_json=False,
         single_cls=False,
         augment=False,
         verbose=False,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            if isinstance(inf_out, torch.Tensor):
                myout = inf_out.cpu().numpy()
            myout = myout[0]
            for i,ele in enumerate(myout):
                if ele[4] < 0.3:
                    myindic.append(i)
            myout = np.delete(myout,myindic,axis=0)
    ...
    print(pf % ('all', seen, nt.sum(), mp, mr, map50, map,myloss))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/exp70/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='data/boll.yaml', help='*.data path')
