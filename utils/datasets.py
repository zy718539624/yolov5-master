...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if os.path.isfile(p):  # file
                    ...
                    f += glob.iglob(p + os.sep + '*.*')  #f是一个列表，里面是所有的图片路径
                else:
                    raise Exception('%s does not exist' % p)
            self.img_files = sorted(   #self.img_files 也是所有的图片路径
        ...
        nb = bi[-1] + 1  # number of batches batch的数量，几个batch
        ...
        if self.rect:
            ...
            tmp = np.ceil(np.array(shapes) * img_size / stride + pad).astype(np.int)
            tmp = np.ceil(tmp / 2)*2
            self.batch_shapes = tmp * stride
